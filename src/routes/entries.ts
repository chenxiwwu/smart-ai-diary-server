...
router.put('/:date', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { date } = req.params;
    const { insight, todos, expenses, media } = req.body;
    ...
    if (!entry) {
      ...
      db.prepare(
        'INSERT INTO entries (id, user_id, date, insight) VALUES (?, ?, ?, ?)'
      ).run(entryId, userId, date, insight || '');
      entry = { id: entryId };
    } else {
      // Update existing entry
      db.prepare(
        'UPDATE entries SET insight = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?'
      ).run(insight || '', entry.id);
    }
    ...
    const updatedTodos = db.prepare('SELECT * FROM todos WHERE entry_id = ?').all(entry.id);
    const updatedExpenses = db.prepare('SELECT * FROM expenses WHERE entry_id = ?').all(entry.id);
    const updatedMedia = db.prepare('SELECT * FROM media WHERE entry_id = ?').all(entry.id);
    ...
    res.json({
      entry: {
        ...
        todos: updatedTodos.map(t => ({ id: t.id, text: t.text, completed: !!t.completed })),
        expenses: updatedExpenses.map(e => ({ id: e.id, item: e.item, amount: e.amount })),
        media: updatedMedia.map(m => ({ id: m.id, type: m.type, url: m.url, name: m.name })),
...
router.delete('/:date', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
    ...
    db.prepare('DELETE FROM entries WHERE user_id = ? AND date = ?').run(req.user!.id, date);
